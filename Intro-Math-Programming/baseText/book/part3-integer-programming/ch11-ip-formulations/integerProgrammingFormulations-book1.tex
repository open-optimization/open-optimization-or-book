...
A company produces two products, $x_1$ and $x_2$. Production of product~1 requires a special machine that can either run in mode~A or mode~B, but not both simultaneously.  The constraints are:
\begin{itemize}
...
A factory can produce a product in any quantity $x \geq 0$, but starting up production incurs a fixed cost of $\$500$.  The variable production cost is $\$8$ per unit, and each unit sells for $\$15$.  The factory has capacity to produce at most 200 units.
