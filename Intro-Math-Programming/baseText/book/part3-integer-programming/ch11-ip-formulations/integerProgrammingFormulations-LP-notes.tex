...
\begin{figure}[H]
\begin{center}
\includegraphics[scale = 0.2]{optimization/figures/figures-static/wiki/File/knapsack}\footnotemark 
\end{center}
\label{fig:knapsack}
...
\end{figure}
\footnotetext{Image from Wikipedia, \url{https://en.wikipedia.org/wiki/Knapsack_problem} (\url{https://commons.wikimedia.org/wiki/File:Knapsack.svg}), licensed CC BY-SA 2.5; see file page for author.}
...
\begin{center}
\includegraphics[scale = 0.75]{Figures/facility-location}\par\captionof{figure}{A network diagram for facility location: three potential distribution centers connected to stores.}\label{fig:facility-location-b}% fig-num-auto
...
\begin{align*}
\min \quad & \displaystyle\sum_{i=1}^n\sum_{j=1}^mc_{ij}z_{ij}+\sum_{i=1}^nf_ix_i \\
\st \quad  & \displaystyle\sum_{i=1}^nz_{ij}=1 &&\quad \text{for all } j=1,\dots,m \\
& \displaystyle \sum_{j=1}^mz_{ij}\leqslant Mx_i &&\quad \text{for all } i=1\dots,n \\
& z_{ij}\in\{0,1\} &&\quad \text{for all } i=1,\dots,n \text{ and } j=1,\dots,m\\
...
\min \quad & \displaystyle\sum_{i=1}^n\sum_{j=1}^mc_{ij}z_{ij}+\sum_{i=1}^nf_ix_i \\
\st \quad  & \displaystyle\sum_{i=1}^nz_{ij}=1 &&\quad \text{for all } j=1,\dots,m \\
& z_{ij}\leqslant x_i &&\quad \text{for all } i=1\dots,n \text{ for all } j = 1, \dots, m \\
& z_{ij}\in\{0,1\} &&\quad \text{for all } i=1,\dots,n \text{ and } j=1,\dots,m\\
& x_i\in\{0,1\} &&\quad \text{for all } i=1,\dots,n
\end{align*}
\newpage
...
\begin{align*}
\min \quad & \one^\top x\\
\st \quad  & \sum_{v \in V_i} x_v \geq 1 &&\quad \text{for all } i =1, \dots, l \\ 
...
\begin{align*}
\min \quad & \one^\top x\\
\st \quad  & x_u + x_v \geq 1 &&\quad \text{for all } (u,v) \in E \\ 
& x_v \in \{0,1\} &&\quad \text{for all } v \in V.
\end{align*}
\end{general}
...
\st \quad  & \sum_{j=1}^n x_{i j}=1 \quad \forall i \in V, \\
...
\bottomrule
\end{tabular}
\par\captionof{table}{Directed arcs between the boroughs and whether each crossing charges a toll.}\label{tab:integerProgrammingFormulations-LP-notes-tab01}% tab-num-auto
...
Let $(x_i,y_i)$ denote the position of the lower-left-hand corner of each rectangle $i \in I$. The overlapping constraint \eqref{mod:StripPackingOverlap-simple} is the trickiest part. Consider a pair of rectangles $i$ and $j$: rectangle $j$ is located entirely to the left of rectangle $i$ if $x_j$ has a value larger than $x_j+w_j$. That is, if $x_j \geq x_j+w_j$. On the other hand, if $y_j \geq y_i+h_i$, then rectangle $j$ is located entirely above rectangle $i$. If either of these constraints is satisfied then rectangles $i$ and $j$ do not overlap. We could also place $i$ above or to the right of $j$. This gives four constraints that we need to satisfy at least one of. The model can be thought of thusly:
	...
	\begin{subequations}
	\begin{align}
	\min \quad & \max_{i\in I}\{y_i+h_i\} 	\label{mod:StripPackingObj-simple}\\
		\st \quad  & \max_{i\in I}\{x_i+w_i\} \leq W 						\label{mod:StripPackingWidth-simple}\\
				   & \left.\begin{aligned}
				   		x_i+w_i &\leq x_j \\
				   		x_j+w_j &\leq x_i \\
				   		y_i+h_i &\leq y_j \\
...
Constraint \eqref{mod:StripPackingOverlap-simple} is a set of four \textit{disjunctive} constraints. This can be expressed linearly by introducing a new binary indicator variable $\delta_{ijk}$ for each of the disjunctive constraints in \eqref{mod:StripPackingOverlap-simple}:
	\begin{align}
	\min \quad & H \nonumber\\
		\st \quad  & y_i+h_i \leq H && \forall\ i\in I \tag{\ref{mod:StripPackingObj-simple}}\\
				   & x_i+w_i \leq W && \forall\ i\in I \tag{\ref{mod:StripPackingWidth-simple}}\\
				   & x_i+w_i \leq x_j+M(1-\delta_{ij1}) && \forall\ i,j\in I:\ i>j \nonumber \\
				   & x_j+w_j \leq x_i+M(1-\delta_{ij2}) && \forall\ i,j\in I:\ i>j \nonumber \\
				   & y_i+h_i \leq y_j+M(1-\delta_{ij3}) && \forall\ i,j\in I:\ i>j \tag{\ref{mod:StripPackingOverlap-simple}}\\
				   & y_j+h_j \leq y_i+M(1-\delta_{ij4}) && \forall\ i,j\in I:\ i>j \nonumber \\
				   & \sum_{k=1}^4\delta_{ijk} \geq 1 && \forall\ i,j\in I:\ i>j \nonumber \\
				   & x_i,y_i \geq 0 && \forall\ i\in I \nonumber \\
				   & \boldsymbol{\delta}_{ij} \in\{0,1\}^4 && \forall\ i,j\in I:\ i>j \nonumber 
	\end{align}
...
$$
\begin{center}
\includegraphics[scale = 1]{Figures/pwl-plot.pdf}\par\captionof{figure}{Piecewise linear function.}\label{fig:pwl-plot-b}% fig-num-auto
...
\begin{align*}
\max \quad & y\\
\st \quad  & y \leq x_i \ \text{ for }\  i=1, \dots, n \\
& x \in X
\end{align*}
...
\st \quad  & x_i = y_i^2 + z_i^2 \text{ for } i=1, \dots, n
\end{align*}
...
% Provenance note (2026-07-16): an earlier draft of the next three subsections
% (exact absolute value / exact 1-norm / exact maximum) followed the MOSEK
% Modeling Cookbook closely (legacy file optimization/
% integerProgrammingFormulations-LP-notes.tex still carries the "%Borrowed
% from Mosek Cookbook" comment). The cookbook is plain "(c) MOSEK ApS" - NOT
% confirmed under an open license. The prose below is a rewrite; the
% formulations themselves are standard (methods are not copyrightable).
% Resolved 2026-07-31: cookbook cited as further reading at the end of the
% exact-maximum subsection below.
...
For further reading on these and many related mixed-integer modeling patterns, see the MOSEK Modeling Cookbook\footnote{MOSEK ApS, \emph{MOSEK Modeling Cookbook}, \url{https://docs.mosek.com/modeling-cookbook/}.}, which surveys conic and integer reformulations of nonsmooth functions.
...
\begin{align*}
\min \quad & C_{\max} \\
\st \quad  & s_{o^{j}_{r}j} \geq s_{o^{j}_{r-1}j} +p_{o^{j}_{r-1}j} && \forall j \in {J}, r \in \{2,\dots,m\} && \text{follow the route of job $j$} \\
...
This section is adapted from the Wikipedia article \emph{Generalized assignment problem}\footnote{Wikipedia contributors, ``Generalized assignment problem,'' \url{https://en.wikipedia.org/wiki/Generalized_assignment_problem}, licensed CC~BY-SA; text edited and reformatted for this book.}.
In applied mathematics, the maximum
\textbf{generalized assignment problem} is a problem in
combinatorial optimization. This
problem is a generalization of the
assignment problem in which both tasks and
agents have a size. Moreover, the size of each
...
assignment problem. When the costs and
...
then, this problem reduces to the knapsack problem.
...
\st \quad  & \sum_{j=1}^n w_{ij} x_{ij} \le t_i && i=1, \ldots, m; \\
