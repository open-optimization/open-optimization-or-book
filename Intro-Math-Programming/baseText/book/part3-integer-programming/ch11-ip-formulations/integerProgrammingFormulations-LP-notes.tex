...
\begin{align}
    \min \quad & \sum_{t=1}^{T} \left( p_t x_t + f_t y_t + h_t s_t \right)  & \text{(Minimize total cost)} \label{eq:objective} \\
    \text{s.t.} \quad & s_{t-1} + x_t - d_t = s_t, & \forall t = 1,2,\dots,T  \quad \text{(Inventory balance)} \label{eq:inventory} \\
    & x_t \leq y_t c_t,  & \forall t = 1,2,\dots,T  \quad \text{(Capacity and setup constraint)} \label{eq:capacity} \\
    & x_t, s_t \geq 0,  & \forall t = 1,2,\dots,T  \quad \text{(Non-negativity constraints)} \label{eq:nonnegativity} \\
    & y_t \in \{0,1\},  & \forall t = 1,2,\dots,T  \quad \text{(Binary production decision)} \label{eq:binary}
\end{align}
...
\text{s.t.} \quad & \sum_{i=1}^n x_{ij} = d_j \quad \text{for all } j = 1,\dots,m \tag{Meet demand} \\
