...
% [enigma images removed: files absent from repo, provenance unknown]
For some fascinating history of computation and code breaking, see the story of Alan Turing: \url{https://en.wikipedia.org/wiki/Alan_Turing}.
...
%\end{multline}
\begin{example}{Verifying a Big-O bound}{}
\label{ex:bigO}
...
\begin{table}[h]
    ...
    \begin{tabular}{|c|c|c|c|c|}
        ...
        \hline
    \end{tabular}
\caption{Growth of $f(n)=n^2+n$ compared with the upper bounds $g(n)=2n^2$ and $h(n)=2n^3$.}\label{tab:complexity-tab01}% tab-num-auto
\end{table}
...
\hline
\end{tabular}
\par\captionof{table}{Common classes of functions in Big-O notation.}\label{tab:complexity-tab02}% tab-num-auto

\end{center}
\begin{center}
\includefigurebook[][scale = 0.15][h]{time-of-algorithms}
\end{center}
...
\end{figure}
\includefigurebook[Runtime comparison of sorting algorithms: the quadratic growth of Bubble, Insertion, and Selection sort dominates the near-linear growth of the efficient algorithms.][scale = 0.35][h]{sorting-quadratic}

\includefigurebook[][scale = 0.35][h]{sorting-linear}

\includefigurebook[][scale = 0.35][h]{bubble-sort-complexity-2}
...
\includefigurebook[][scale = 0.35][h]{bubble-sort-computational-example}
...
\end{itemize}
It is not known if $P$ is the same as NP, but it is conjectured that these are very different classes.  This would mean that the NP-Hard problems (and NP-Complete problems) are necessarily much more difficult than the problems in P.  See Figure~\ref{fig:complexity-classes}.
% Original TikZ (2026-07-31) replacing a deleted Wikipedia image.
\begin{center}
\begin{tikzpicture}[scale=0.85, every node/.style={font=\small}]
  % ---- Left: P != NP ----
  \begin{scope}
    \draw[rounded corners=14pt, fill=orange!12] (-2.1,-1.5) rectangle (2.1,2.9);
    \node at (0,2.55) {NP-hard};
    \draw[fill=blue!10] (0,0.55) ellipse (1.75 and 1.85);
    \node at (0,-1.05) {NP};
    \draw[fill=green!15] (0,-0.15) ellipse (1.0 and 0.75);
    \node at (0,-0.15) {P};
    \draw[fill=red!15] (0,1.85) ellipse (1.0 and 0.5);
    \node at (0,1.85) {NP-complete};
    \node at (0,-2.0) {$P \neq NP$};
  \end{scope}
  % ---- Right: P = NP ----
  \begin{scope}[xshift=6.2cm]
    \draw[rounded corners=14pt, fill=orange!12] (-2.1,-1.5) rectangle (2.1,2.9);
    \node at (0,2.55) {NP-hard};
    \draw[fill=green!15] (0,0.35) ellipse (1.6 and 1.5);
    \node at (0,0.35) {$P = NP =$ NP-complete};
    \node at (0,-2.0) {$P = NP$};
  \end{scope}
\end{tikzpicture}
\par\captionof{figure}{The two possibilities for complexity classes. Most
researchers believe the left picture, $P \neq NP$, in which NP-complete
problems sit inside NP but outside P, and NP-hard problems extend beyond
NP.}\label{fig:complexity-classes}% fig-num-auto
\end{center}
...
Integer programming (IP) offers a powerful modeling framework capable of representing a wide range of optimization problems, including fundamental ones like the sorting problem. In this section, we will delve into an IP formulation to sort a given list of numbers.
...
\subsubsection{VERTEX COVER to SET COVER}
...
The key idea is that a perfect matching in the 3D matching problem corresponds to a satisfying assignment in the 3SAT problem.
...
For example,
$$
\neg \true \lor \false \lor \true = \true \qquad \true \land \neg \true = \false .
$$
...
\includefigurebook[][scale = 0.5][h]{graph-for-matching-maximal}
...
\includefigurebook[][scale = 0.5][h]{spanning-tree}
...
Graphs with nice properties are often easier to handle and typically graphs found in the real world have some nice properties. The \emph{triangle inequality} comes from the idea of a triangle that the sum of the lengths of two sides always are longer than the length of the third side. See Figure~\ref{fig:triangle-inequality}.

\begin{center}
\begin{tikzpicture}[scale=1.1]
  \coordinate (TA) at (0,0);
  \coordinate (TB) at (4.6,0.4);
  \coordinate (TC) at (2.0,2.9);
  \draw[thick] (TA) -- (TB) node[midway,below,sloped] {\small $d(A,B)$};
  \draw[thick] (TB) -- (TC) node[midway,above right] {\small $d(B,C)$};
  \draw[thick,blue!70!black] (TA) -- (TC) node[midway,above left] {\small $d(A,C)$};
  \fill (TA) circle (1.6pt) node[below left] {$A$};
  \fill (TB) circle (1.6pt) node[below right] {$B$};
  \fill (TC) circle (1.6pt) node[above] {$C$};
  \node at (2.3,-1.0) {$d(A,C) \le d(A,B) + d(B,C)$};
\end{tikzpicture}
\par\captionof{figure}{The triangle inequality: the direct distance $d(A,C)$ is never longer than the detour through $B$.}\label{fig:triangle-inequality}% fig-num-auto
\end{center}
