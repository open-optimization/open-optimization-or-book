...
\end{tikzpicture}\par\captionof{figure}{Eighteen-vertex housing-development graph with nodes A through R laid out on a roughly grid-like...}\label{fig:graphtheory-dor1-tikz01}% fig-num-auto
...
\begin{tikzpicture}
 ...
 \foreach \source/ \dest in {A/D, D/G, A/B, B/E, B/C, C/F, E/F, F/M, D/E, E/G, G/H, H/I, H/J, I/K, J/K, K/L, N/J, F/N, F/M, N/O, O/P, P/Q, Q/R, M/O, K/P, L/R}
              \path (\source) edge (\dest);                
\end{tikzpicture}\par\captionof{figure}{The same 18-vertex graph G (vertices A through R) redrawn with vertices scattered in a non-grid layout.}\label{fig:graphtheory-dor1-tikz02}% fig-num-auto
...
\end{tikzpicture}\par\captionof{figure}{Schematic of Konigsberg's four landmasses drawn as labelled ellipses (North Bank, Island, East...}\label{fig:graphtheory-dor1-tikz03}% fig-num-auto
...
\end{tikzpicture}\par\captionof{figure}{Graph abstraction of the Konigsberg bridges: four circular vertices labelled NB, I, EB, and SB with...}\label{fig:graphtheory-dor1-tikz04}% fig-num-auto
...
\draw(1.5,0)--(1,-1);  
 ...
 \end{tikzpicture}\par\captionof{figure}{Small graph illustrating a loop: three filled-dot vertices form a triangle and a red self-loop...}\label{fig:graphtheory-dor1-tikz05}% fig-num-auto
...
\draw(1,2)--(2,2);
\draw(2,2)--(3,2);
\end{tikzpicture}\par\captionof{figure}{A 3-by-4 grid graph with 12 vertices labelled A-D (top), E-H (middle), J-M (bottom).}\label{fig:graphtheory-dor1-tikz11}% fig-num-auto
...
\draw(1,2)--(2,2);
\draw(2,2)--(3,2);
\end{tikzpicture}\par\captionof{figure}{The same 3-by-4 grid graph (vertices A-D, E-H, J-M) with a circuit highlighted in thick red...}\label{fig:graphtheory-dor1-tikz12}% fig-num-auto
...
\draw(0,2)--(1,2);
\draw(2,2)--(3,2);
\end{tikzpicture}\par\captionof{figure}{A 3-by-4 grid graph with vertices A-D, E-H, J-M, but with the column-A horizontal edges deleted so...}\label{fig:graphtheory-dor1-tikz13}% fig-num-auto
...
\draw(6,-2)--(8,0);
\node at (7.5,-1){\$85};
 \end{tikzpicture}\par\captionof{figure}{Weighted graph of five U.S. cities (LA, Seattle, Chicago, Atlanta, Dallas) shown as filled dots...}\label{fig:graphtheory-dor1-tikz14}% fig-num-auto
...
\end{tikzpicture}\par\captionof{figure}{Initial-state diagram for Dijkstra's algorithm: seven blue circular nodes T, E, A, NB, MR, P, Y...}\label{fig:graphtheory-dor1-tikz15}% fig-num-auto
...
\draw(4,0)--(8,0);
\node at (6,0.3){96};
\end{tikzpicture}\par\captionof{figure}{Weighted road network for the Yakima-to-Tacoma example: seven filled-dot vertices T (Tacoma), E...}\label{fig:graphtheory-dor1-tikz16}% fig-num-auto
...
\draw(4,0)--(8,0);
\node at (6,0.3){96};
\end{tikzpicture}\par\captionof{figure}{Yakima-to-Tacoma road network as before, now with the destination Y annotated [0] in red to mark it...}\label{fig:graphtheory-dor1-tikz17}% fig-num-auto
...
\draw(4,0)--(8,0);
\node at (6,0.3){96};
\end{tikzpicture}\par\captionof{figure}{Yakima example after Dijkstra step 2: tentative distances [104] at NB, [96] at MR, [76] at P are...}\label{fig:graphtheory-dor1-tikz18}% fig-num-auto
...
\draw(4,0)--(8,0);
\node at (6,0.3){96};
\end{tikzpicture}\par\captionof{figure}{Yakima example after the second iteration: P is the current vertex; E now has tentative distance...}\label{fig:graphtheory-dor1-tikz19}% fig-num-auto
...
\draw(4,0)--(8,0);
\node at (6,0.3){96};
\end{tikzpicture}\par\captionof{figure}{Yakima example after the third iteration: A has tentative distance [175] (via MR); P and Y appear...}\label{fig:graphtheory-dor1-tikz20}% fig-num-auto
...
\draw(4,0)--(8,0);
\node at (6,0.3){96};
\end{tikzpicture}\par\captionof{figure}{Yakima example after the fourth iteration: A's distance is updated to [140] via NB; NB, MR, P, Y...}\label{fig:graphtheory-dor1-tikz21}% fig-num-auto
...
\draw(4,0)--(8,0);
\node at (6,0.3){96};
\end{tikzpicture}\par\captionof{figure}{Yakima example after the fifth iteration: T receives tentative distance [160] via A; A, NB, MR, P...}\label{fig:graphtheory-dor1-tikz22}% fig-num-auto
...
\node at (0,0.9){\$11};
\node at (3.4,0.85){\$14};
\end{tikzpicture}\par\captionof{figure}{A 4-vertex spanning tree drawn as filled dots at corners of a small region, connected by three...}\label{fig:graphtheory-dor1-tikz23}% fig-num-auto
...
\draw(0,0)--(1,1);
\draw(1,2)--(2,0);
\end{tikzpicture}\par\captionof{figure}{A 4-vertex spanning tree with three edges arranged as a path: middle vertex linked to top-right...}\label{fig:graphtheory-dor1-tikz24}% fig-num-auto
...
\draw(2,0)--(1,1);
\draw(1,2)--(2,0);
\end{tikzpicture}\par\captionof{figure}{Composite figure of five small spanning trees with n = 1, 2, 3, 4, 5 vertices arranged...}\label{fig:graphtheory-dor1-tikz25}% fig-num-auto
...
\node at (0,0.9){\$11};
\node at (3.4,0.85){\$14};
\end{tikzpicture}\par\captionof{figure}{Five-vertex graph with cities A, B, C, D, E as black dots, fully connected with dollar-cost edge...}\label{fig:graphtheory-dor1-tikz32}% fig-num-auto
...
\end{tikzpicture}\par\captionof{figure}{Undirected weighted graph with six yellow circular vertices A, B, C, E, F, G arranged hexagonally.}\label{fig:graphtheory-dor1-tikz35}% fig-num-auto
...
\draw(6,2)--(6,-2);
\node at (6.3,0){2};
\end{tikzpicture}\par\captionof{figure}{Boxed figure for Prim's algorithm: 7-vertex graph with vertices a, b, c, d, e, f, g as red circles.}\label{fig:graphtheory-dor1-tikz39}% fig-num-auto
...
\draw(1,1)--(2,1);
\draw(2,1)--(1,0);
\end{tikzpicture}\par\captionof{figure}{Seven-vertex weighted graph for the shortest-path exercise.}\label{fig:graphtheory-dor1-tikz40}% fig-num-auto
...
\draw(-1,1)--(-0.5,2);
\draw(1,1)--(0.5,2);
\end{tikzpicture}\par\captionof{figure}{Airfare table giving one-way dollar fares between Seattle, Honolulu, London, Moscow, and Cairo (e.g.}\label{fig:graphtheory-dor1-tikz41}% fig-num-auto
...
\end{tikzpicture}\par\captionof{figure}{European rail network with travel times between cities.}\label{fig:graphtheory-dor1-tikz48}% fig-num-auto
...
\end{tikzpicture}\\\par\captionof{figure}{Weighted graph on five labelled vertices A, B, C, D, E in a pentagonal layout, fully connected by...}\label{fig:graphtheory-dor1-tikz49}% fig-num-auto
...
\end{tikzpicture}\par\captionof{figure}{Virginia cities road network with distances.}\label{fig:graphtheory-dor1-tikz50}% fig-num-auto
