...
Let $x_1, \dots, x_5 \geq 0$ be daily servings of raw carrots, baked potatoes, wheat bread, cheddar cheese, and peanut butter. The LP is
\begin{align*}
\min \quad & 0.14x_1 + 0.12x_2 + 0.20x_3 + 0.75x_4 + 0.15x_5\\
\st & 23x_1 + 171x_2 + 65x_3 + 112x_4 + 188x_5 \geq 2000 && \text{(calories)}\\
& 0.1x_1 + 0.2x_2 + 9.3x_4 + 16x_5 \geq 50 && \text{(fat)}\\
& 0.6x_1 + 3.7x_2 + 2.2x_3 + 7x_4 + 7.7x_5 \geq 100 && \text{(protein)}\\
& 6x_1 + 30x_2 + 13x_3 + 2x_5 \geq 250 && \text{(carbohydrate)}\\
& x_1, \dots, x_5 \geq 0.
\end{align*}
...
The optimal diet is potatoes and peanut butter only:
\[
x_2 \approx 7.7147 \text{ servings of baked potatoes}, \qquad x_5 \approx 9.2800 \text{ servings of peanut butter},
\]
with $x_1 = x_3 = x_4 = 0$ and minimum daily cost $\approx \$2.32$ (exactly $\$2.3178$). The protein and carbohydrate constraints are binding ($100$ g and $250$ g exactly); calories ($\approx 3064$) and fat ($\approx 150$ g) are comfortably oversatisfied. A screenshot should show the yellow serving cells with these values and the green cost cell at $2.32$.

\begin{qaflag}
Licensing: this exact food table (raw carrots, baked potatoes, wheat bread, cheddar cheese, peanut butter, with these prices and nutrient values and the 2000/50/100/250 requirements) circulates widely in university course materials and closely matches the diet-problem exercise found in at least one commercial optimization textbook. Recommend verifying the original source; if it cannot be traced to an openly licensed source, regenerate the table with fresh (verified) nutrition data, which would not change the pedagogical content. The numbers themselves are fine: the LP is feasible, non-degenerate, and has a clean two-food solution.
\end{qaflag}
