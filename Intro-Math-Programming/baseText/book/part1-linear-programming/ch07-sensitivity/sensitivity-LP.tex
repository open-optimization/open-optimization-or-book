...
\stars{2} A shop assembles two products with profits \$3 and \$5 per unit:
\[
\begin{aligned}
\max \quad & 3x_1 + 5x_2 \\
\st \quad
& x_1 + 2x_2 \leq 14 \quad \text{(assembly hours)} \\
...
\]
so the shadow prices are $\mathbf{y}^\top = \mathbf{c}_B^\top A_B^{-1} = [3, 5]\,A_B^{-1} = \left[\tfrac94, \tfrac14\right]$: an assembly hour is worth \$2.25 and a machine hour \$0.25.
\item With $\mathbf{b} = (b_1, 18)$, feasibility of $\mathbf{x}_B = A_B^{-1}\mathbf{b} = \left(-\tfrac{b_1}{2} + 9,\ \tfrac{3b_1}{4} - \tfrac{9}{2}\right) \geq \mathbf{0}$ gives $6 \leq b_1 \leq 18$, with $z = 36 + \tfrac94(b_1 - 14)$ inside the range.  With $\mathbf{b} = (14, b_2)$, feasibility of $\left(\tfrac{b_2}{2} - 7,\ \tfrac{21}{2} - \tfrac{b_2}{4}\right) \geq \mathbf{0}$ gives $14 \leq b_2 \leq 42$, with $z = 36 + \tfrac14(b_2 - 18)$.
\item With $\mathbf{c}_B = (c_1, 5)$ the dual prices are $\left(-\tfrac{c_1}{2} + \tfrac{15}{4},\ \tfrac{c_1}{2} - \tfrac{5}{4}\right)$; both nonnegative exactly when $\tfrac52 \leq c_1 \leq \tfrac{15}{2}$.  With $\mathbf{c}_B = (3, c_2)$ the dual prices are $\left(\tfrac{3c_2}{4} - \tfrac32,\ \tfrac32 - \tfrac{c_2}{4}\right)$, giving $2 \leq c_2 \leq 6$.
\item Software confirms: at $b_1 = 6$ the solution is $(6,0)$ with $z = 18 = 36 + \tfrac94(-8)$, at $b_1 = 18$ it is $(0,9)$ with $z = 45 = 36 + \tfrac94(4)$, and just outside these values the duals change, signaling a new basis.
\end{enumerate}
\end{solution}
