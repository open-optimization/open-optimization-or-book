...
feasible starting dictionary --- i.e., setting the original variables
...
We run the simplex method on the running example, colour-coding each computation to match the algorithm card above: {\color{algenter}\textbf{entering}} (green), {\color{algleave}\textbf{ratio test / leaving}} (orange), {\color{algpivot}\textbf{pivot}} (blue), and {\color{algopt}\textbf{optimal}} (purple).
...
\begin{definition}{Reduced Costs in an Arbitrary Dictionary}\label{def:def:reduced-cost-general}
...
\st\quad & x = 2.50 - 0.50y + 0.50s_1 - 0.50a_1,\\[1mm]
& s_2 = 11 - s_1 + a_1,\\[1mm]
...
\begin{aligned}
\max\quad & 5 + 2y + s_1 \\
\st\quad & x = 2.50 - 0.50y + 0.50s_1,\\[1mm]
& s_2 = 11 - s_1,\\[1mm]
...
\begin{tcolorbox}[colback=algenter!6, colframe=algenter, coltitle=white, fonttitle=\bfseries, sharp corners,
    width=0.6\textwidth, % adjust width as desired
    box align=center,
    title={\ding{43}\ First Feasible Iteration --- Choose entering variable}]

We choose $y$ to enter the basis since it has the largest positive coefficient in the objective of $2$.
\end{tcolorbox}
\begin{ex}{Complete the Simplex Method}{}
...
\begin{tcolorbox}[colback=sensbox!5!white, colframe=sensbox!75!black,
    width=0.6\textwidth, % adjust width as desired
    box align=center,
...
\]
\end{tcolorbox}
\textbf{[Pivot the artificial variable into the basis to create a feasible basis]} 
...
\end{aligned}
\]
Two constraints are of ``$\ge$'' type, so the origin violates both and we will need two artificial variables.

\begin{steppanel}{algslate}
\cstep{1}{Rewrite in ``$\le$'' form and build the dictionary} Flipping the two $\ge$ constraints gives $-2x-y\le-5$ and $-x-2y\le-3$, and the slack dictionary is
\[
\begin{aligned}
s_1 &= -5 + 2x + y, \\
s_2 &= 16 - 2x - y, \\
...
\end{aligned}
\]
At the origin $s_1=-5<0$ and $s_3=-3<0$, so two equations lack a valid basic variable.
\end{steppanel}

\begin{steppanel}{algslate}
\cstep{2}{Introduce one artificial variable per offending constraint} Add $a_1$ to the $s_1$ equation and $a_3$ to the $s_3$ equation, each penalized by $M=1000$ in the objective:
\[
\begin{aligned}
\max\quad & 3x + y - 1000a_1 - 1000a_3 \\
\st\quad
& s_1 = -5 + 2x + y + a_1, \\
& s_2 = 16 - 2x - y, \\
& s_3 = -3 + x + 2y + a_3, \\
...
\begin{steppanel}{algslate}
\cstep{3}{Pivot the artificial variables into the basis} Swapping $a_1$ with $s_1$ and $a_3$ with $s_3$ produces a feasible starting dictionary (every right-hand side is nonnegative at $x=y=0$). Substituting the artificials into the objective gives its Big-M form:
\[
\begin{aligned}
...
The large penalties make $x$ and $y$ attractive to increase, and successive pivots drive both artificial variables out of the basis. Once $a_1=a_3=0$ the basis is feasible for the original problem, and we discard the artificial variables and continue improving the true objective $3x+y$ until every reduced cost is nonpositive. This yields the optimal dictionary
\[
\begin{aligned}
...
\end{aligned}
\]
Every objective coefficient is now nonpositive, so we stop. Setting the nonbasic variables $y=s_2=0$ gives the optimal solution
\[
x = 8,\quad y = 0,\quad z = 3x + y = 24,
\]
with the first and third constraints slack ($s_1=11$, $s_3=5$) and the second constraint tight ($s_2=0$).
...
\begin{proof}
The key idea behind the proof is that Bland's Rule ensures that the sequence of bases never repeats. By always choosing the smallest indexed variable for entry and exit, the algorithm guarantees a strictly lexicographic improvement in the sequence of dictionaries. Since there are a finite number of possible bases, the algorithm must eventually terminate.
...
\[
\begin{aligned}
\max\quad & 0.20 - 4.80x_2 + 1.40s_1 - 2.20s_2 - 0.20s_3 \\
\st\quad & x_3 = 1 - s_3,\\ 
& x_4 = 0.10 + 0.27x_2 - 0.13s_1 + 0.07s_2 - 0.10s_3,\\ 
& x_1 = 0.40 + 22.40x_2 + 0.80s_1 - 2.40s_2 - 0.40s_3,\\ 
