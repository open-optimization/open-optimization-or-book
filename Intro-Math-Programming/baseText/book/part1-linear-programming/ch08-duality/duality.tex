...
\c^\top \x^* = \b^\top \y^*.
\]
\item  It is also possible for both (P) and (D) to be infeasible; in that case neither problem has a solution or a bound to offer the other.
\end{enumerate}
\end{theorem}
...
\begin{example}{The Bakery Revisited}{}
...
\begin{example}{A Larger Example: Bakery 2.0}{}
...
\begin{example}{Example with Inequalities and Free Variables}{}
...
\bottomrule
\end{tabular}
\par\captionof{table}{Correspondence between primal constraint and variable types and their dual counterparts.}\label{tab:duality-tab01}% tab-num-auto
