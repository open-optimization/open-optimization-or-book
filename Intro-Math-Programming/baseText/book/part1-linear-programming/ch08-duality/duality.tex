...
The whole argument is a pipeline that starts from the constraints you already have and ends at a brand-new optimization problem.  Figure~\ref{fig:duality-pipeline} summarizes it.  The key realization is in the last step: searching for the \emph{best} certificate is itself a linear program.
...
\begin{theorem}{Weak Duality}\label{thm:weak-duality-intro}
...
\paragraph{Case 2: Minimization with $\leq$ constraints.}
...
& x_1 + x_2 \leq 12 \quad \text{(oven capacity)} \\
& x_1, x_2 \geq 0
\end{aligned}
\]
