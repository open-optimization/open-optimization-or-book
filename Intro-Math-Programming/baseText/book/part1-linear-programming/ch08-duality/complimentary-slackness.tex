...
\begin{itemize}
\item \textbf{Flour: } \( 5x_1 + 4x_2 + x_3 = 5(4) + 4(0) + 6 = 26 \). This is strictly less than 40, so the constraint is not tight \(\Rightarrow\) corresponding dual variable \(y_1 = 0\).
\item \textbf{Sugar: } \( x_1 + x_2 + x_3 = 4 + 0 + 6 = 10 \). Tight \(\Rightarrow\) \(y_2\) can be positive.
\item \textbf{Baking Time: } \( x_1 + x_2 + 0.5x_3 = 4 + 0 + 3 = 7 \). Tight \(\Rightarrow\) \(y_3\) can be positive.
\end{itemize}

\textbf{Step 2: Check tightness of dual constraints where \(x_j > 0\)}

\begin{itemize}
\item \(x_1 = 4 > 0\): The corresponding dual constraint must be tight:
\[
5y_1 + y_2 + y_3 = 0 + 50 + 40 = 90
\]
\item \(x_2 = 0\): No condition required.
\item \(x_3 = 6 > 0\): The dual constraint for muffins must be tight:
\[
...
Substitute back into (1'): \( y_2 + 40 = 90 \Rightarrow y_2 = 50 \)

\textbf{Result: } \( y_1 = 0,\ y_2 = 50,\ y_3 = 40 \)
...
\[
\text{maximize} \quad 90x_1 + 40x_2 + 70x_3
\]

Subject to the following resource constraints:
\begin{itemize}
  \item \textbf{Flour ($\leq$ 40 kg):} \quad $5x_1 + 4x_2 + x_3 \leq 40$
  \item \textbf{Sugar ($\leq$ 10 kg):} \quad $x_1 + x_2 + x_3 \leq 10$
  \item \textbf{Baking Time ($\leq$ 7 hrs):} \quad $x_1 + x_2 + 0.5x_3 \leq 7$
...
\[
\text{minimize} \quad 40y_1 + 10y_2 + 7y_3
\]
\[
\text{subject to} \quad
\begin{aligned}
...
\begin{enumerate}
  ...
  Using the dual solution $(y_1, y_2, y_3)$, interpret the shadow price of each resource (flour, sugar, baking time). For example, what does $y_2 = 50$ tell you about the value of an additional kg of sugar?
...
\hline
\end{tabular}
\par\captionof{table}{The six meal kit options and the profit per kit.}\label{tab:complimentary-slackness-tab01}% tab-num-auto
...
\hline
\end{tabular}
\par\captionof{table}{Kitchen and packaging time required per meal kit.}\label{tab:complimentary-slackness-tab02}% tab-num-auto
...
\hline
\end{tabular}
\par\captionof{table}{Packaging set types and retail prices.}\label{tab:complimentary-slackness-tab03}% tab-num-auto
...
\hline
\end{tabular}
\par\captionof{table}{Profit per unit for each packaging set.}\label{tab:complimentary-slackness-tab04}% tab-num-auto
...
\hline
\end{tabular}
\par\captionof{table}{Labor and material required per packaging set.}\label{tab:complimentary-slackness-tab05}% tab-num-auto
...
\text{s.t.} \quad
& 2x_1 + x_2 \le 90 && \text{(Labor)} \\
& x_1 + x_2 \le 70 && \text{(Material)} \\
& x_1, x_2 \ge 0
\end{aligned}
\]
\end{minipage}
...
\textbf{Solver Output:}
\begin{itemize}
    \item Optimal solution: \( x_1 = 45,\; x_2 = 0 \), profit \$2250
    \item Shadow prices:
    \begin{itemize}
        \item Labor: \$25
        \item Material: \$0
