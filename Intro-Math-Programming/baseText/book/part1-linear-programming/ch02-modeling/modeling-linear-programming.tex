...
The final assumption asserts that the coefficients (e.g., profit per plane or boat) are known with absolute certainty. In traditional linear programming, there is no lack of knowledge about the makeup of the objective function, the coefficients in the left-hand-side of the constraints or the bounds on the right-hand-sides of the constraints. There is a literature on \textit{stochastic programming} that relaxes some of these assumptions, but this too is outside the scope of this book.
...
\stars{2} A chemical manufacturer produces three chemicals: A, B and C. These chemicals are produced by two processes: 1 and 2.
\begin{itemize}
\item Running process 1 for 1 hour costs \$4 and yields 3 units of chemical A, 1 unit of chemical B and 1 unit of chemical C.
\item Running process 2 for 1 hour costs \$1 and produces 1 unit of chemical A, and 1 unit of chemical B (but none of Chemical C).
\end{itemize}
...
\stars{2} An appliance store sells three types of ovens: small, medium, and large. The small, medium, and large ovens require, respectively, 3, 5, and 6 cubic feet of storage space; at most 90 cubic feet of storage is available. Each oven takes 1 hour of sales time; there is a maximum of 200 hours of sales labor time available. The small, medium, and large ovens require, respectively, 1, 1, and 2 hours of installation time; at most 280 hours of installer labor are available monthly.
...
\stars{2} A company produces three products, A, B, and C, at two factories, Factory I and Factory II. Daily production of each factory for each product is listed below:
...
\begin{ex}{Stochastic Objective}{}
\stars{2} Suppose the costs are not known with certainty but instead a probability distribution for each value of $c_i$ ($i=1,\dots,n$) is known. Suggest a way of constructing a linear program from the probability distributions.
%
...
\exrefs{\S\ref{sec:lp-assumptions}}
\end{ex}
...
Let $x_1$ and $x_2$ be the number of hours that Process 1 and Process 2 are run each day.  Each hour of Process 1 costs \$4 and each hour of Process 2 costs \$1, so we minimize total cost subject to meeting each chemical demand:
\begin{align*}
\min \quad & z = 4x_1 + x_2\\
...
& x_1, x_2 \geq 0.
\end{align*}
Since only Process 1 produces chemical C, the demand for C forces $x_1 \geq 3$.  Solving the LP gives $x_1 = 3$, $x_2 = 2$ with minimum daily cost \$14: running Process 1 for exactly 3 hours covers chemical C and most of A, and 2 hours of the cheaper Process 2 fills the remaining demand for B.
\end{solution}
...
Let $x_A, x_B, x_C$ be the number of units of products A, B, and C manufactured per month.  Each machine's hours give one constraint:
\begin{align*}
...
& x_A, x_B, x_C \geq 0.
\end{align*}
An optimal solution is $x_A = 120$, $x_B = 30$, $x_C = 0$ with maximum profit \$3300.  Both machine constraints are binding, and it is not worth producing product C: its profit does not justify the machine time it consumes compared with A and B.
\end{solution}

\begin{solution}(Exercise~\ref{ex:factory-days})
Let $x_1$ and $x_2$ be the number of days that Factory I and Factory II operate.  We minimize operating cost subject to filling the order:
\begin{align*}
...
& x_1, x_2 \geq 0.
\end{align*}
The optimal solution is $x_1 = 60$ days and $x_2 = 20$ days, with minimum cost \$340{,}000.  At this point the constraints for products A and B are binding, while product C is overproduced ($1400 \geq 700$ units).
\end{solution}
