...
\begin{align*}
\min_{x,s}\quad & z = \sum_{t=1}^{T} \Big( c^{prod}_t\, x_t + c^{inv}\, s_t \Big) \quad \text{(Total Cost)} \\[1mm]
\text{s.t.}\quad & s_0 + x_1 = d_1 + s_1, \quad \text{(Period 1 Balance)} \\[1mm]
& s_{t-1} + x_t = d_t + s_t, \quad \forall\, t=2,\dots,T \quad \text{(Inventory Balance)} \\[1mm]
...
\underline{Mathematical Formulation:}
\begin{align*}
\min_{x,s}\quad & z = 10x_1 + 12x_2 + 14x_3 + 16x_4 + 18x_5 + 20x_6 + 22x_7 + 24x_8 + 26x_9 + 28x_{10} \\
& \quad + 5\,(s_1+s_2+s_3+s_4+s_5+s_6+s_7+s_8+s_9+s_{10}) \\[1mm]
\text{s.t.}\quad & 5 + x_1 = 8 + s_1, \quad \text{(Period 1)}\\[1mm]
& s_1 + x_2 = 6 + s_2, \quad \text{(Period 2)}\\[1mm]
& s_2 + x_3 = 9 + s_3, \quad \text{(Period 3)}\\[1mm]
& s_3 + x_4 = 7 + s_4, \quad \text{(Period 4)}\\[1mm]
& s_4 + x_5 = 10 + s_5, \quad \text{(Period 5)}\\[1mm]
& s_5 + x_6 = 5 + s_6, \quad \text{(Period 6)}\\[1mm]
& s_6 + x_7 = 8 + s_7, \quad \text{(Period 7)}\\[1mm]
& s_7 + x_8 = 6 + s_8, \quad \text{(Period 8)}\\[1mm]
& s_8 + x_9 = 4 + s_9, \quad \text{(Period 9)}\\[1mm]
& s_9 + x_{10} = 7 + s_{10}, \quad \text{(Period 10)}\\[1mm]
...
\underline{\textbf{Decision Variables:}}
\begin{itemize}
...
\underline{\textbf{Objective and Constraints:}}
\begin{align*}
\min \quad & z = \sum_{t=1}^{T} \Big( c^{prod}_t\,x_t + c^{over}_{t}\,y_t + c^{inv}\,s_t \Big) \quad \text{(Total Cost)} \\[1mm]
\text{s.t.} \quad & s_0 + x_1 + y_1 = d_1 + s_1 \quad \text{(Day 1 balance)} \\[1mm]
& s_{t-1} + x_t + y_t = d_t + s_t, \quad t = 2,\dots, T \quad \text{(Inventory balance)} \\[1mm]
...
\underline{\textbf{Mathematical Formulation:}}
\begin{align*}
...
& s_9 + x_{10} + y_{10} = 7 + s_{10} \quad \text{(Day 10 balance)} \\[1mm]
...
\medskip
\underline{\textbf{Mathematical Model}}:

\begin{align}
    \min \quad & \sum_{i \in I} \sum_{j \in J} c_{ij} x_{ij} \label{eq:objective} \\
    \text{s.t.} \quad
    & \sum_{j \in J} x_{ij} = 1, && \forall i \in I \quad \text{(Each agent gets exactly one task)} \label{eq:agent_constraint} \\
    & \sum_{i \in I} x_{ij} = 1, && \forall j \in J \quad \text{(Each task is assigned to exactly one agent)} \label{eq:task_constraint} \\
    & x_{ij} \in \{0,1\}, && \forall i \in I, \; j \in J. \label{eq:binary_constraint}
\end{align}

\medskip
\underline{\textbf{Explanation}}:
\begin{itemize}
    \item \textbf{Objective function} \eqref{eq:objective}: Minimizes the total assignment cost based on given costs \( C_{ij} \).
    \item \textbf{Constraints} \eqref{eq:agent_constraint}: Each agent is assigned to exactly one task.
    \item \textbf{Constraints} \eqref{eq:task_constraint}: Each task is assigned to exactly one agent.
    \item \textbf{Binary constraint} \eqref{eq:binary_constraint}: Ensures each assignment is either made (\( x_{ij} = 1 \)) or not (\( x_{ij} = 0 \)).
\end{itemize}
...
\begin{examplewithcode}{School Bus Routing Problem}{\href{https://github.com/open-optimization/open-optimization-or-examples/blob/master/integer-programming/School-bus-assignment.ipynb}{PuLP}}
...
\medskip {\bf Min-max Assignment-like Formulation} \\
\begin{align*}
    \min \;& z  \nonumber \\
       \text{  s.t.}      \;& z \geq \sum_{i \in I} C_{i j} x_{i j},~~ && \forall j \in J  \\
   & \sum_{i \in I} x_{i j} = 1,~~  && \forall j \in J  \\
         & \sum_{j \in J} x_{i j} = 1,~~  && \forall i \in I  \\
         & x_{i j} \ge 0,~~               && \forall i \in I, \; j \in J 
\end{align*}

\medskip {\bf Does this formulation have the integrality property?}  
...
\begin{align}
    S = \min \; & z \\
    \text{s.t.} \ & z \geq \sum_{j \in J} p_{i j} x_{i j},  && \forall i \in I \\
                 & \sum_{j \in J} x_{i j} = 1,            && \forall i \in I \\
                 & \sum_{i \in I} x_{i j} = 1,            && \forall j \in J \\
...
\end{itemize}
This is acheived by the model
\begin{align}
    \min\ & \sum_{i \in I} \sum_{j \in J} p_{i j} x_{i j} \\
    \text{s.t.} \ & \sum_{j \in J} x_{i j} = 1,            && \forall i \in I \\
                 & \sum_{i \in I} x_{i j} = 1,            && \forall j \in J \\
                 & \sum_{j \in J} p_{i j} x_{i j} \leq S, && \forall i \in I \\
                 & x_{i j} \in \{0,1\},                  && \forall i \in I, \; j \in J
\end{align}
