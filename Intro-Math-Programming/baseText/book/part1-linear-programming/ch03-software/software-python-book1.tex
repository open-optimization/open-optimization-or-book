...
For this course, we'll primarily use \textbf{Jupyter Notebooks}. If you haven't encountered them before, Jupyter notebooks are interactive documents that combine:
...
\item \textbf{Free GPU access} -- useful for machine learning (not needed for this course)
...
\begin{resource}
A companion Jupyter notebook \texttt{intro\_to\_python\_extended.ipynb} is available in the course repository's \texttt{code/python/} folder. We recommend working through this notebook interactively as you read this section.
\end{resource}
...
\begin{info}
GeoPandas is optional for this course, but it transforms abstract optimization problems into concrete, visual stories. It's especially valuable when presenting results to stakeholders who may not be familiar with mathematical notation.
\end{info}
...
\begin{warn}
\textbf{Common Mistake:} If you forget the comparison operator (\texttt{<=}, \texttt{>=}, \texttt{==}), you'll silently overwrite the objective function instead of adding a constraint!
