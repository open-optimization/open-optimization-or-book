...
\begin{example}{Proof that a Polyhedron is Convex}\label{exa:polyhedron-convex}
...
\begin{align*}
\mathbf{c}^\top \mathbf{z}  &= \mathbf{c}^\top \mathbf{z} (\lambda \mathbf{x} + (1-\lambda) \mathbf{y}) & \text{Replace formula for $\mathbf{z}$}\\
...
By Lemma~\ref{lemma:objective_bound}, the value of \( \mathbf{a}^\top \mathbf{z} \) satisfies:
\[
