% Adapted from the "Gradient Descent Methods" lab of Foundations of Applied
% Mathematics (Humpherys, Jarvis, et al.), CC BY 3.0 US. See
% external-sources/foundationsAppliedMathematicsLabs/Volume2/GradientMethods.
\chapter{Gradient Descent Methods}
\label{ch:gradient-methods}

\begin{outcome}
\begin{enumerate}
\item Understand line-search methods: choosing a search direction and a step size at each iteration
\item Implement the method of steepest descent, with exact and backtracking step sizes
\item Implement the conjugate gradient method for linear systems and its non-linear extension
\item Apply these methods to regression problems
\end{enumerate}
\end{outcome}

Iterative optimization methods choose a search direction and a step size at each iteration.
...
In this chapter we implement a method of steepest descent and two conjugate gradient methods, then apply them to regression problems.\footnote{This chapter is adapted from the \emph{Gradient Descent Methods} lab of \emph{Foundations of Applied Mathematics} by Humpherys, Jarvis, et al., licensed CC BY 3.0 US.}
...
\EndProcedure
\end{algorithmic}
\caption{The Conjugate Gradient method for solving the positive definite linear system $Q\x = \b$.}
...
\EndProcedure
\end{algorithmic}
\caption{The Non-Linear Conjugate Gradient method (Fletcher--Reeves variant).}
\label{Alg:nonlinear-conjugate-gradient}
\end{algorithm}

\begin{problem}{Non-linear conjugate gradient}{}
Write a function that accepts a convex objective function $f$, its derivative $Df$, an initial guess $\x_0$, a convergence tolerance defaulting to $1e^{-5}$, and a maximum number of iterations defaulting to $100$.
