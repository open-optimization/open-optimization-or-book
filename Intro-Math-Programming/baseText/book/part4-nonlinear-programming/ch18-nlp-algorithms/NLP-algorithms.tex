...
\begin{figure}[H] % Golden section search for unimodal optimization.
    \centering
    \includegraphics[width=.7\textwidth]{part4-nonlinear-programming/OneD_Optimization/figures/golden_section.pdf}
    ...
    Figure from \emph{Foundations of Applied Mathematics, Volume 2} by Humpherys, Jarvis, et al., CC BY 3.0 US.}
...
\begin{figure}[H]
\centering
\includegraphics[width = .7 \textwidth]{part4-nonlinear-programming/OneD_Optimization/figures/quad_approx.pdf}
...
\paragraph{Convergence of Newton's Method}

We consider the function $f(x) = e^x + e^{-x}$, starting from $x^0 = 6.1$. The table below tracks the iterates: the step sizes $|x^k - x^{k-1}|$ barely shrink at first (ratio near $1$) while the iterates are far from the minimizer, and then collapse quadratically once the iterates enter the basin near $x^* = 0$.

\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{c|rrrrr}
\toprule
$k$ & $x^k$ & $f(x^k)$ & $f'(x^k)$ & $|x^k - x^{k-1}|$ & $\dfrac{|x^k - x^{k-1}|}{|x^{k-1} - x^{k-2}|}$ \\
\midrule
$0$ & $6.10000$ & $445.860$ & $445.856$ & --- & --- \\
$1$ & $5.10001$ & $164.030$ & $164.018$ & $0.99999$ & --- \\
$2$ & $4.10008$ & $60.3620$ & $60.3288$ & $0.99993$ & $0.99994$ \\
$3$ & $3.10063$ & $22.2570$ & $22.1670$ & $0.99945$ & $0.99953$ \\
$4$ & $2.10468$ & $8.32635$ & $8.08258$ & $0.99595$ & $0.99650$ \\
$5$ & $1.13396$ & $3.42968$ & $2.78617$ & $0.97072$ & $0.97467$ \\
$6$ & $0.32159$ & $2.10431$ & $0.65432$ & $0.81237$ & $0.83687$ \\
$7$ & $1.06\times 10^{-2}$ & $2.00011$ & $2.13\times 10^{-2}$ & $0.31094$ & $0.38276$ \\
$8$ & $4.02\times 10^{-7}$ & $2.00000$ & $8.04\times 10^{-7}$ & $1.06\times 10^{-2}$ & $3.42\times 10^{-2}$ \\
$9$ & $\approx 0$ & $2.00000$ & $\approx 0$ & $4.02\times 10^{-7}$ & $3.78\times 10^{-5}$ \\
\bottomrule
\end{tabular}
\par\captionof{table}{Convergence of Newton's method on $f(x) = e^x + e^{-x}$ from $x^0 = 6.1$: slow progress far from the minimizer, then quadratic convergence near $x^* = 0$.}\label{tab:NLP-algorithms-newton-convergence}% tab-num-auto
\end{center}
...
\section{Multi-Variate Unconstrained Optimization}
...
\begin{aligned}
\min \quad & s\\
\st \quad  & f_i(x) \leq s & \text{for } i=1, \dots, m\\
& x \in \R^d, s \in \R
\end{aligned}
\end{equation}
...
\begin{equation}
\label{eq:convex-programming-barrier-initial}
\begin{aligned}
...
\paragraph{Complexity} The total work of the barrier method can be bounded by multiplying the number of outer iterations (how many times the barrier parameter must be increased by the factor $\mu$ before $m/t \leq \epsilon$) by a bound on the number of Newton steps needed at each centering. This gives a bound of the form
\[
N=\left\lceil\frac{\log \left(m /\left(t^{(0)} \epsilon\right)\right)}{\log \mu}\right\rceil\left(\frac{m(\mu-1-\log \mu)}{\gamma}+c\right)
\]
Newton steps in total, where $\gamma$ and $c$ depend on the backtracking and tolerance parameters of the inner Newton solver. See Boyd and Vandenberghe, \emph{Convex Optimization} (Cambridge University Press, 2004), Section~11.5, for the derivation and a discussion of how the bound depends on the choice of $\mu$.


\begin{refsection}
\begin{pioneerspotlight}{Margaret H. Wright}
Margaret H. Wright was born in Hanford, California, in 1944 \parencite{riddle_2025_margaret}. She attended Stanford University where she got a Bachelor of Science in Mathematics, a Master of Science in Computer Science, and in 1976, returned to get a Doctor of Philosophy in Computer Science after working in industry. Wright was hired by a fellow operations researcher, George Dantzig, where she worked in a lab and taught classes \parencite{informs_2025_wright}. After her time at Stanford, she left academia to join Bell Laboratories’ Computer Science Center. She rejoined academia when she became a professor in Computer Science at the Courant Institute at New York University in 2001. In 1997, she was chosen for the National Academy of Engineering, together with two other societies, in 2001 and 2005 \parencite{riddle_2025_margaret}. She wrote two books on optimization and had at least 40 publications. As a professor in a male-dominated field, she would encourage women and minority students to find opportunities within the industry.

Margaret predominately researched interior methods, a method once overshadowed by the simplex method and barrier method \parencite{wright_2004_the}. Wright described the connection of linear and nonlinear programming after Karmarkar’s method broke the separated fields. Interior methods can solve continuous nonlinear optimization problems by determining the set of feasible points. Rather than traversing the boundaries of the feasible region with the simplex method, the interior method iterates inside the feasible region and moves towards the most optimal vertex without regard to the boundaries \parencite{lauterio_2022_interiorpoint}. The interior method utilizes the primal-dual pairs, the Lagrange Function, which includes constraints, and Newton’s Method for an unconstrained minimization system.

According to \textcite{lauterio_2022_interiorpoint}, interior methods can solve larger problems faster than the simplex method. Wright’s development of the interior method has been applied in power flow optimization problems to reduce operational cost while increasing user efficiency. In addition, there are uses in hydroelectric power systems for similar purposes. More recently, primal-dual interior point methods have been used in image processing, such as deblurring. 
