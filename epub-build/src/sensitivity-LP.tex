...
\bottomrule
\end{tabular}
\par\captionof{table}{Effect of each type of data change on the basic solution and the objective value.}\label{tab:sensitivity-LP-tab01}% tab-num-auto
...
\begin{steppanel}{sensbox}
\sslab{Adapt the final dictionary}
The optimal dictionary is valid with \( s_1' \) in place of \( s_1 \); substituting \( s_1' = s_1 - \Delta \) expresses it in the original variables:
\[
\begin{aligned}
\max \quad
...
\begin{steppanel}{sensbox}
\sslab{Adapt the final dictionary}
The optimal dictionary is valid with \( s_3' \) in place of \( s_3 \); substituting \( s_3' = s_3 - \Delta \) expresses it in the original variables:
\[
\begin{aligned}
\max \quad
...
\bottomrule
\end{tabular}
\par\captionof{table}{Allowed perturbation ranges and their effect on the objective value.}\label{tab:sensitivity-LP-tab02}% tab-num-auto
...
\includegraphics[width=3.87in]{figs/sensitivity-LP-tikz01.png}
\end{center}
...
\begin{center}
\altincludegraphics[scale = 0.3]{Figures/sensitivity-objective}{Feasible region in the x-y plane with several level curves of the objective z = c_x x + 3y drawn for different slopes. The figure shows that as long as c_x lies in the interval [1.5, 3], the level curves tilt within the cone defined by the active constraints at the current optimum, so the optimal vertex (and basis) does not change.}\par\captionof{figure}{Feasible region with level curves of the objective $z = c_x x + 3y$ for several slopes: for $c_x \in [1.5, 3]$ the optimal vertex (and basis) does not change.}\label{fig:sensitivity-objective}% fig-num-auto
...
\begin{center}
\altincludegraphics[scale =0.3]{Figures/excel-sensitivity}{Screenshot of Excel's Solver Results dialog with the option to generate a Sensitivity report selected, illustrating how to request post-optimal sensitivity output after solving a linear program.}\par\captionof{figure}{Excel's Solver Results dialog with the option to generate a Sensitivity report selected.}\label{fig:excel-sensitivity}% fig-num-auto
...
\item \textbf{Constraint Status:} The report shows which constraints are binding (active) and which are non-binding. A binding constraint restricts the optimal solution and typically has a positive shadow price (under degeneracy a binding constraint can have a shadow price of zero), while a non-binding constraint always has a shadow price of zero.
\end{itemize}
...
\bottomrule
\end{tabular}
\par\captionof{table}{Variable section of the Solver sensitivity report.}\label{tab:sensitivity-LP-tab03}% tab-num-auto
...
\bottomrule
\end{tabular}
\par\captionof{table}{Constraint section of the Solver sensitivity report.}\label{tab:sensitivity-LP-tab04}% tab-num-auto
