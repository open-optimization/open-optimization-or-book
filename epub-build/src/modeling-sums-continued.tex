...
\begin{center}
\includegraphics[width=3.51in]{figs/modeling-sums-continued-tikz01.png}\par\captionof{figure}{Undirected network with six numbered circular nodes: edges join 6-4, 4-5, 4-3, 5-1, 5-2, 3-2, and 2-1.}\label{fig:modeling-sums-continued-tikz01}% fig-num-auto
...
\end{itemize}
\begin{center}
\includegraphics[width=3.51in]{figs/modeling-sums-continued-tikz02.png}\par\captionof{figure}{Directed version of the same six-node network: curved arrows show arc directions, including a...}\label{fig:modeling-sums-continued-tikz02}% fig-num-auto
...
The network data is summarized in the following table:
    ...
    \begin{tabular}{|c|c|c|}
        ...
        \hline
    \end{tabular}
\par\captionof{table}{Transportation cost and capacity for each route.}\label{tab:modeling-sums-continued-tab01}% tab-num-auto
...
Solving the linear program, we obtain the following optimal flow values:
    ...
    \begin{tabular}{|c|c|}
        ...
        \hline
    \end{tabular}
\par\captionof{table}{Optimal flow on each route.}\label{tab:modeling-sums-continued-tab02}% tab-num-auto
...
\vspace{0.5em}
\begin{example}{Amazonian's Distribution Network}{}
...
\vspace{-1em}
\begin{center}
\includegraphics[width=3.13in]{figs/modeling-sums-continued-tikz03.png}\par\captionof{figure}{Transshipment network: supply nodes W1 (+30) and W2 (+20) on the left, transshipment nodes D1 and...}\label{fig:modeling-sums-continued-tikz03}% fig-num-auto
...
\]
\begin{center}
\includegraphics[width=3.96in]{figs/modeling-sums-continued-tikz04.png}\par\captionof{figure}{Directed network with eight nodes labeled a through h arranged left to right.}\label{fig:modeling-sums-continued-tikz04}% fig-num-auto
...
\begin{itemize}
    ...
    \item \( A = \{(s,a), (s,b), (s,c), (a,c), (a,d), (b,c), (b,e), (c,d), (c,e), (c,t), (d,t), (e,t)\} \): set of flight legs.
...
& x_{dt} - x_{ad} - x_{cd} = 0 & \text{(at \( d \))} \\
& x_{et} - x_{be} - x_{ce} = 0 & \text{(at \( e \))} \\
...
& 0 \leq x_{cd} \leq 1,\quad 0 \leq x_{ce} \leq 6,\quad 0 \leq x_{ct} \leq 4 \\
...
\((3,4)\) &  9 & 1 \\
\end{tabular}
\par\captionof{table}{Arc capacities and costs for the two-commodity network.}\label{tab:modeling-sums-continued-tab03}% tab-num-auto
...
This model makes two simplifying assumptions, which we state explicitly: cash that is not invested in a period is \emph{not} carried forward (each period's budget is exactly the previous period's payouts), and the objective counts only the period-4 payouts. A more realistic model would introduce a cash-balance variable for each period that carries un-invested funds forward and adds them to the final total; we leave that extension as an exercise.
...
Formulate a max-flow optimization problem for this scenario. Define your decision variables, objective function, and constraints explicitly.  Assume that you do not want to leave any inventory at the distribution centers.
    ...
    \begin{tabular}{|c|c|c|c|c|}
        ...
        \hline
    \end{tabular}
\par\captionof{table}{Node data for the distribution network: type, inventory, connections, and capacity.}\label{tab:modeling-sums-continued-tab04}% tab-num-auto

    
\begin{center}
\includegraphics[width=\linewidth]{figs/modeling-sums-continued-tikz10.png}\par\captionof{figure}{Disaster-relief transportation network: warehouse rectangles W1-W5 (inventories 200, 300, 150, 250...}\label{fig:modeling-sums-continued-tikz10}% fig-num-auto
...
\begin{center}
    \includegraphics[width=2.49in]{figs/modeling-sums-continued-tikz11.png}\par\captionof{figure}{Directed max-flow exercise network: green source s and red sink t with intermediate nodes v1, v2, v3, v4.}\label{fig:modeling-sums-continued-tikz11}% fig-num-auto
...
\begin{center}
    \includegraphics[width=2.94in]{figs/modeling-sums-continued-tikz12.png}\par\captionof{figure}{Six-node directed network for a min-cost flow exercise: nodes v1 through v6 with blue supply values...}\label{fig:modeling-sums-continued-tikz12}% fig-num-auto
...
$H_2$ & 5 & 3 & 2 \\
\end{tabular}
\par\captionof{table}{Per-unit shipping costs from factories to hubs (left) and from hubs to stores (right).}\label{tab:modeling-sums-continued-tab05}% tab-num-auto
