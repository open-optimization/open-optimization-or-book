...
\begin{tryit}
\vizlink{simplex-tableau}{Simplex Pivoter in Tableau Form}: the same pivoting mechanics as this chapter, with every row operation shown.

\vizlink{tableau-pivoter}{Free Tableau Pivoter}: a Gauss--Jordan sandbox; pivot on any entry and see what happens.
\end{tryit}
...
\end{tabular}
\end{center}
the basis is $\{s_1,s_2,s_3\}$. Setting the nonbasic variables $x=y=0$ gives the basic feasible solution $s_1=9,\ s_2=16,\ s_3=14$ read directly from the RHS column, with objective value $z=0$ (the RHS of the $z$-row). The entries of the $z$-row, $-2$ and $-3$, record the \emph{reduced costs} of $x$ and $y$ with the sign flipped: increasing $x$ or $y$ from zero would change $z$ at the rates $+2$ and $+3$, the negatives of the stored entries.
...
\bottomrule
\end{tabular}
\par\captionof{table}{Interpreting the final Big-$M$ tableau: the artificial variable leaves the basis versus stays basic.}\label{tab:simplex-tableau-tab01}% tab-num-auto
...
\begin{ex}{Reading a mid-solve tableau}{}
\label{ex:tableau-read-mid}
\stars{1} The tableau below is the second tableau of this chapter's running example, reached after one pivot:
\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{c|ZZZZZ|Z}
\toprule
...
\end{center}
\begin{enumerate}
    \item Which variables are basic and which are nonbasic?
    \item Read off the basic feasible solution and the objective value.
    \item What are the reduced costs of the nonbasic variables?
    \item Is this tableau optimal?  If not, which variable enters next?
\end{enumerate}
\exrefs{\S\ref{sec:tableau-building}, first worked tableau; \S\ref{sec:tableau-pivot}}
\end{ex}

\begin{ex}{Reading a tableau}{}
\label{ex:tableau-reading}
\stars{1} The following tableau arose while solving a maximization problem, with the objective row on top:
\begin{center}
\renewcommand{\arraystretch}{1.3}
...
\stars{2} Starting from the tableau in Exercise~\ref{ex:tableau-ratio-test}, perform the pivot you identified and write the resulting tableau. Verify that the entering column has become a unit column and that the objective value in the $z$-row has increased.
\exrefs{\S\ref{sec:tableau-pivot}}
\end{ex}

\begin{ex}{Solve a linear program in tableau form}{}
\label{ex:tableau-solve-lp}
\stars{2} Use the simplex method in tableau form to solve
\[
\begin{aligned}
\max\quad & z = x + 2y\\
\st\quad  & x + y  \le 4,\\
...
\begin{ex}{A full two-pivot solve}{}
\label{ex:tableau-two-pivot-solve}
\stars{2} Solve the following linear program in tableau form, using the most-negative-entry rule to choose the entering variable:
\[
\begin{aligned}
\max\quad & z = 3x + 2y\\
\st\quad  & 2x + y  \le 10,\\
...
\stars{2} Consider the linear program
\[
\begin{aligned}
\max\quad & 4x + 3y \\
...
\begin{ex}{The $z$-row sign convention}{}
\label{ex:tableau-sign-convention}
\stars{2} The dictionary form of Chapter~\ref{ch:simplex-dictionary} selects an entering variable with a \emph{positive} objective coefficient, while the tableau selects one with a \emph{negative} $z$-row entry.
\begin{enumerate}
    \item Explain where the sign flip comes from, using the fact that the tableau stores the objective as the equation $z - \c^\top\x = 0$.
    \item A dictionary reads $z = 12 + 3x_1 - 2x_2$.  Write the corresponding $z$-row of a tableau, including its RHS entry.
    \item Explain why the RHS entry of the $z$-row equals the current objective value even though every other coefficient is negated.
\end{enumerate}
\exrefs{\S\ref{sec:tableau-building}, sign-convention panel}
\end{ex}

\begin{ex}{From tableau to dictionary}{}
\label{ex:tableau-to-dictionary}
\stars{2} Write the dictionary that corresponds to the tableau in Exercise~\ref{ex:tableau-reading}, expressing $z$ and each basic variable in terms of the nonbasic variables $s_1$ and $s_2$. Confirm that setting the nonbasic variables to zero reproduces the same basic feasible solution.
\exrefs{\S\ref{sec:tableau-building}; Chapter~\ref{ch:simplex-dictionary}}
\end{ex}

\begin{ex}{Recognizing an unbounded problem}{}
\label{ex:tableau-unbounded}
\stars{2} While solving a maximization problem you obtain
\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{c|ZZZZ|Z}
\toprule
...
\exrefs{\S\ref{sec:tableau-optimality}, ratio test step of the algorithm card}
\end{ex}

\begin{ex}{Reading off infeasibility}{}
\label{ex:tableau-infeasible}
\stars{2} Suppose the Big-M method applied to some maximization problem terminates (every objective-row entry nonnegative) with the tableau
\begin{center}
\renewcommand{\arraystretch}{1.3}\small\setlength{\tabcolsep}{4.5pt}
\begin{tabular}{c|ccccc|c}
\toprule
...
\begin{ex}{Certifying unboundedness from a tableau}{}
\label{ex:tableau-unbounded-certificate}
\stars{3} Applying the simplex method to
\[
\begin{aligned}
\max\quad & z = x_1 + x_2\\
...
\end{center}
\begin{enumerate}
    \item Detect the unboundedness: identify the entering variable and show that the ratio test finds no limiting row.
    \item Certify it: letting the entering variable equal $t \ge 0$, write the resulting family of solutions $\x(t)$ in all four variables, and verify by direct substitution that $\x(t)$ satisfies both original constraints and the nonnegativity conditions for every $t \ge 0$.
    \item Show that $z(t) \to \infty$ as $t \to \infty$, and sketch the feasible region to explain geometrically which direction the certificate ray points in.
\end{enumerate}
\exrefs{\S\ref{sec:tableau-optimality}; Example~\ref{exa:simplex-unbounded}}
\end{ex}

\subsubsection*{Selected Solutions}

\begin{solution}(Exercise~\ref{ex:tableau-reading})
The basic variables are the row labels $x_1$ and $x_2$; the nonbasic variables are $s_1$ and $s_2$.  Setting the nonbasic variables to zero and reading the RHS column gives the basic feasible solution
\[
x_1 = 6, \quad x_2 = 4, \quad s_1 = s_2 = 0, \qquad z = 40.
\]
The objective-row entries of the nonbasic variables are $3$ and $1$, both nonnegative.  In this convention a positive entry means that increasing the corresponding nonbasic variable would \emph{decrease} $z$, so no pivot can improve the objective and the tableau is optimal.
\end{solution}

\begin{solution}(Exercise~\ref{ex:tableau-ratio-test})
The most negative objective-row entry is $-4$, so $x_1$ enters.  The ratio test divides each RHS by the positive entries of the $x_1$ column:
\[
\frac{6}{2} = 3, \qquad \frac{8}{4} = 2, \qquad \frac{12}{3} = 4.
\]
The minimum ratio $2$ occurs in the $s_2$ row, so $s_2$ leaves and the pivot element is the $4$ in the $s_2$ row of the $x_1$ column.
\end{solution}

\begin{solution}(Exercise~\ref{ex:tableau-solve-lp})
Adding slacks $s_1, s_2$, the initial tableau has objective row $(-1, -2, 0, 0 \mid 0)$.

\textbf{First pivot:} $y$ enters ($-2$ is most negative).  Ratios: $4/1 = 4$ and $6/3 = 2$, so $s_2$ leaves; the pivot element is the $3$ in the $s_2$ row.  Dividing the pivot row by $3$ and clearing the $y$ column gives
\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{c|ZZZZ|Z}
\toprule
...
\textbf{Second pivot:} $x$ enters ($-\tfrac13$).  Ratios: $2/\tfrac23 = 3$ and $2/\tfrac13 = 6$, so $s_1$ leaves; the pivot element is the $\tfrac23$ in the $s_1$ row.  The resulting tableau is
\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{c|ZZZZ|Z}
\toprule
...
\begin{solution}(Exercise~\ref{ex:tableau-two-pivot-solve})
Adding slacks $s_1, s_2$, the starting tableau is
\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{c|ZZZZ|Z}
\toprule
...
\textbf{First pivot:} $x$ enters ($-3$ is most negative).  Ratios: $10/2 = 5$ and $15/1 = 15$, so $s_1$ leaves; the pivot element is the $2$ in the $s_1$ row.  Scaling the pivot row by $\tfrac12$ and clearing the $x$ column gives
\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{c|ZZZZ|Z}
\toprule
...
\textbf{Second pivot:} $y$ enters ($-\tfrac12$).  Ratios: $5/\tfrac12 = 10$ and $10/\tfrac52 = 4$, so $s_2$ leaves; the pivot element is the $\tfrac52$ in the $s_2$ row.  The resulting tableau is
\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{c|ZZZZ|Z}
\toprule
...
\end{tabular}
\end{center}
Every $z$-row entry is nonnegative, so the tableau is optimal: $(x, y) = (3, 4)$ with $z^* = 17$ and $s_1 = s_2 = 0$ (both constraints are tight at the optimum).
\end{solution}
