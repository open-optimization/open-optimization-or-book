...
\begin{center}
\includegraphicbook[scale = 0.75]{facility-location}\par\captionof{figure}{A network diagram for facility location: three potential distribution centers connected to stores.}\label{fig:facility-location}% fig-num-auto
...
Graph coloring \refincludefigurestatic[A valid 3-coloring of the Petersen graph: no two adjacent vertices share a color.]{wiki-File-Petersen-graph-3-coloring.png}% fig-num-auto
 is
...
\bottomrule
\end{tabular}
\par\captionof{table}{Directed arcs between the boroughs and whether each crossing charges a toll.}\label{tab:integerProgrammingFormulations-book1-tab01}% tab-num-auto
...
$$
\begin{center}
\includegraphicbook[scale = 1]{pwl-plot}\par\captionof{figure}{Piecewise linear function.}\label{fig:pwl-plot}% fig-num-auto
...
\begin{example}{Job Shop Scheduling Problem Input Data}{}
...
\caption{Job shop scheduling solution shown as a Gantt chart.}\label{fig:jssp}% fig-num-auto
\end{figure}
...
\begin{center}
\includetikz[width = 0.8\textwidth]{jssp-duplo}\par\captionof{figure}{Gantt chart of the Duplo job shop scheduling solution with three machines and four color-coded jobs.}\label{fig:jssp-duplo}% fig-num-auto

%\includegraphicstatic[scale = 0.5]{jssp-duplo}

\includegraphicbook[scale=0.07, angle=180]{jssp-duplo-actual}\par\captionof{figure}{Photograph of Duplo building blocks arranged as a physical demonstration of the job shop schedule.}\label{fig:jssp-duplo-actual}% fig-num-auto
...
Camera & 2 & 4 \\
\end{tabular}
\par\captionof{table}{Weights and values of the items available to the hiker.}\label{tab:integerProgrammingFormulations-book1-tab02}% tab-num-auto
...
\end{tabular}
\par\captionof{table}{Package weights and payouts for the courier van.}\label{tab:integerProgrammingFormulations-book1-tab03}% tab-num-auto
...
D & 180 & 3, 4, 5 \\
\end{tabular}
\par\captionof{table}{Fixed opening costs and regions served for each candidate warehouse.}\label{tab:integerProgrammingFormulations-book1-tab04}% tab-num-auto
...
\end{tabular}
\par\captionof{table}{Net present values and cash outlays for the five candidate projects (in \$1000s).}\label{tab:integerProgrammingFormulations-book1-tab05}% tab-num-auto
...
\bottomrule
\end{tabular}
\par\captionof{table}{Opening costs, capacities, per-unit serving costs, and store demands for the two candidate centers.}\label{tab:integerProgrammingFormulations-book1-tab06}% tab-num-auto
...
\end{itemize}
At least one of these two constraints must hold (the machine operates in one of the two modes; the big-$M$ formulation enforces at least one, which is what the model requires). The objective is to maximize $5x_1 + 4x_2$ with $x_1, x_2 \geq 0$.
...
Job 2 & 2 & 4 & 3 \\
\end{tabular}
\par\captionof{table}{Processing times for the two jobs on the three machines.}\label{tab:integerProgrammingFormulations-book1-tab07}% tab-num-auto
...
\]
The optimal load is packages $2, 3, 4, 5$ with weight $3+2+5+4 = 14 \leq 15$ and payout $3+3+7+4 = \$17$.  The heavy package 1 never pays: taking it leaves only 4\,kg of room, and the best it can then achieve is $5 + 4 = \$9$.
\end{solution}
