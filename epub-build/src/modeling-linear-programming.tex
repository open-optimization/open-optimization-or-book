...
\begin{itemize}
    ...
    \item $a_{ij}$: Constraint coefficients, $\forall i = 1,\dots,m$ and $j = 1,\dots,n$. The first index $i$ identifies the constraint and the second index $j$ identifies the variable; $a_{ij}$ is the coefficient of $x_j$ in constraint $i$.
    \item $b_i$: Right-hand side values of the constraints, $\forall i = 1,\dots,m$. These represent limits or requirements for each constraint.
\end{itemize}
...
\begin{example}{Example of a Linear Program}{}
...
\begin{itemize}
    ...
    \item $a_{ij}$: Constraint coefficients, $\forall i = 1,\dots,4$ and $j = 1,\dots,3$ ($a_{ij}$ is the coefficient of $x_j$ in constraint $i$).
    \item $b_i$: Right-hand side values for each constraint, $\forall i = 1,\dots,4$.
\end{itemize}
...
\begin{description}
\item[Proportionality] Doubling a variable doubles its effect.  If one t-shirt earns \$10 and consumes 2 hours of press time, then 50 shirts earn \$500 and consume 100 hours.  No quantity discounts, no economies of scale, no setup costs: every unit behaves like the first one.

\item[Additivity] Effects add up across variables, with no interactions.  The press time used by t-shirts and the press time used by tote bags simply sum; producing one product does not make another product cheaper, more profitable, or harder to make.
...
\includegraphics[width=3.43in]{figs/modeling-linear-programming-tikz01.png}\par\captionof{figure}{Schematic of a two-period production planning network: two gray circles labelled 1 and 2.}\label{fig:modeling-linear-programming-tikz01}% fig-num-auto
...
\end{tabular}
\end{center}
\caption{Units of each nutrient in, and cost of, each tablet type.}\label{tab:pillContent}% tab-num-auto
\end{table}
...
\hline
\end{tabular} \end{center} \caption{Units of each nutrient per gram, calories, and cost of each chemical.}\label{tab:modeling-linear-programming-tab02}% tab-num-auto
\end{table}
...
\begin{itemize}
\item K. Peters, S. Silva, R. Gon\origcedilla{c}alves, M. Kavelj, H. Fleuren, D. den Hertog, O. Ergun, M. Freeman. ``The Nutritious Supply Chain: Optimizing Humanitarian Food Assistance.'' \emph{INFORMS Journal on Optimization} 3(2), 200--226, 2021. \url{https://doi.org/10.1287/ijoo.2019.0047}
\end{itemize}
\end{casestudybox}
...
\hline  6 = Saturday & 7  \\
\hline  7 = Sunday & 7  \\
\hline \end{tabular} \end{center} \caption{Minimum number of workers required at LP Burger each day of the week.}\label{tab:modeling-linear-programming-tab03}% tab-num-auto
\end{table}
...
Unlike the original LP Burger model, which minimizes the number of workers hired, here we minimize total \emph{worker-days}: each full-time worker contributes 5 days and each part-time worker 2 days. (Minimizing the head count $\sum_i x_i + \sum_i y_i$ would be a reasonable alternative objective.)
...
\hline \end{tabular} \end{center} \caption{Weights and values of the candidate items for the camping trip.}\label{tab:modeling-linear-programming-tab04}% tab-num-auto
\end{table}
...
\hline \end{tabular} \end{center} \caption{Required investment and projected return for each candidate project.}\label{tab:modeling-linear-programming-tab05}% tab-num-auto
\end{table}
...
\hline
\end{tabular} \end{center} \caption{Production cost at each factory and shipping cost to each warehouse (in \$ per panel).}\label{tab:modeling-linear-programming-tab10}% tab-num-auto
\end{table}
...
\begin{tabular}{llll}
    ...
    \end{tabular}
\par\captionof{table}{Cost of assigning each person to each task, in dollars.}\label{tab:modeling-linear-programming-tab06}% tab-num-auto
...
\end{array}
\]
\par\captionof{table}{Time requirements (hours per unit) and profit per unit for each product.}\label{tab:modeling-linear-programming-tab07}% tab-num-auto
...
\end{array}
\]
\par\captionof{table}{Cases of each juice bottled per day of operation at each plant.}\label{tab:modeling-linear-programming-tab08}% tab-num-auto
...
\end{array}
\]
\par\captionof{table}{Minimum number of baristas needed each day of the week.}\label{tab:modeling-linear-programming-tab09}% tab-num-auto
