...
\begin{example}{Summing over a set}{}
...
\begin{center}
\includegraphics[width=\linewidth]{figs/modeling-sums-tikz01.png}\par\captionof{figure}{Schematic of a multi-period production network: gray circles labelled 1, 2, ..., T-1, T.}\label{fig:modeling-sums-tikz01}% fig-num-auto
...
\bottomrule
\end{tabular}
\par\captionof{table}{Production planning data for the ten-day horizon.}\label{tab:modeling-sums-tab01}% tab-num-auto
...
\st        & 5 + x_1 = 8 + s_1   \tag{Period 1} \\
                 ...
                 & x_t \geq 0,\ s_t \geq 0 \text{ for all } t=1,\dots,10.
\end{align*}
...
\bottomrule
\end{tabular}
\par\captionof{table}{Production planning data with overtime for the ten-day horizon.}\label{tab:modeling-sums-tab02}% tab-num-auto
...
\st        & s_0 + x_1 + y_1 = 8 + s_1   \tag{Day 1 balance} \\
             ...
             & s_9 + x_{10} + y_{10} = 7 + s_{10}   \tag{Day 10 balance} \\[1mm]
...
\includegraphics[width=2.19in]{figs/modeling-sums-tikz02.png}\par\captionof{figure}{Bipartite assignment diagram: three people P1, P2, P3 drawn as stick-figure icons in blue circles...}\label{fig:modeling-sums-tikz02}% fig-num-auto
...
\begin{examplewithallcode}{Machine Assignment}
...
For example, Machine 0 might perform Job 1 very efficiently, while it would take much longer and cost more to complete Job 2.
...
The cost matrix \(c_{ij}\) reflects the expected operating cost (in dollars) for assigning Machine \(i\) to Job \(j\).  In this instance the costs are
\[
c = \begin{bmatrix} 9 & 2 & 7 & 8 \\ 6 & 4 & 3 & 7 \\ 5 & 8 & 1 & 8 \\ 7 & 6 & 9 & 4 \end{bmatrix},
\]
and the optimal assignment (machine $0 \to$ job $1$, $1 \to 0$, $2 \to 2$, $3 \to 3$) has total cost $13$.


\modelhead{Sets}
\begin{itemize}
\item Let $I = \{0,1,2,3\}$ be the set of machines.
\item Let $J = \{0,1, 2,3\}$ be the set of jobs.
\end{itemize}
...
\bottomrule
\end{tabular}
\par\captionof{table}{Cost of assigning each bus to each school.}\label{tab:modeling-sums-tab03}% tab-num-auto
...
Here $z$ is a lower bound on the total score $\sum_{j \in J} C_{ij} x_{ij}$ of every team $i$, and maximizing $z$ makes the worst team's score as large as possible.
\begin{align*}
\max \quad & z  \nonumber \\
\st        & z \leq \sum_{j \in J} C_{i j} x_{i j},~~ &&\quad \text{for all } i \in I \\
...
\begin{align*}
\min \quad & z  \nonumber \\
\st        & z \geq \sum_{j \in J} C_{i j} x_{i j},~~ &&\quad \text{for all } i \in I \\
   & \sum_{i \in I} x_{i j} = 1,~~ &&\quad \text{for all } j \in J \\
         & \sum_{j \in J} x_{i j} = 1,~~ &&\quad \text{for all } i \in I \\
         & x_{i j} \ge 0,~~ &&\quad \text{for all } i \in I, \; j \in J
\end{align*}
...
\end{tabular} 
\end{center} 
\caption{Rankings given by the two teams for the two projects (lower is better).}\label{tab:modeling-sums-tab04}% tab-num-auto
\end{table}

Both teams prefer Project 2.

If we remove the binary restriction on the \(x\)-variable, the values can range between 0 and 1.  
For the original assignment problem (minimizing total cost), fractional \(x\)-values do not improve the objective: the optimum remains 3, achieved by an integral assignment in which the worst-off team incurs cost 2.  

For the min-max assignment problem, however, this is not the case. The optimal solution has \(z=1.5\), occurring when each team is assigned half of each project (i.e., for Team 1, we have \(x_{11} = 0.5\) and \(x_{12} = 0.5\)). By fractionalizing assignments, the worst-case team cost is minimized.
