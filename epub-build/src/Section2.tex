...
This optimal point sits at the intersection of the two constraints $2x_1+x_2=80$ and $x_1+3x_2=90$. Both constraints are satisfied with equality at the optimum; we say they are \textit{binding}. The remaining constraints ($x_1\leq 32$, $x_1\geq 0$, $x_2\geq 0$) are satisfied strictly and are \textit{non-binding}.
...
Suppose the workshop in Example~\ref{ex:FurnitureWorkshop} revises its pricing so that each chair earns \$2 and each table earns \$1, and the demand limit on chairs is raised from 32 to 35 per week. The new objective function is $z(x_1,x_2) = 2x_1 + x_2$, and the model becomes:
\begin{equation}
\begin{aligned}
...
\begin{solution}
The gradient of the new objective is $\nabla z = (2,1)$, which is normal (perpendicular) to the constraint line $2x_1 + x_2 = 80$; equivalently, the level curves of the objective are parallel to that constraint. As we sweep the level curves in the gradient direction, the last level curve that touches the feasible region coincides with the entire edge from $(30,20)$ to $(35,10)$.  Along this edge, $2x_1+x_2 = 80$ for every point, so $z = 80$ everywhere on it.
...
\begin{ex}{Finite or Unbounded?}{}\label{exer:BoundedSolutionQuestion}
...
\end{tabular}
\caption{Costs and nutrient content for the three products.}
\label{tab:diet-problem-data}
\end{table}
...
Since both bounds must hold simultaneously, for any fixed \(y\) the
smallest feasible value of \(z\) is
\(\max \{ 2+\frac{1}{2}y,\; 1-2y\}\). Minimizing this over \(y\),
the two bounds balance where \(2+\frac{1}{2}y = 1-2y\), which occurs at
...
\begin{center}  \includegraphics[width=3.65in]{figs/Section2-tikz11.png}\par\captionof{figure}{Cone-shaped feasible region between the lines s1 = 0 and s3 = 0, with s2 = 0 above, shaded blue and marked...}\label{fig:Section2-tikz12}% fig-num-auto
 \end{center}
\end{minipage}
...
\medskip To find the extreme directions we can change the right-hand-side to $\mathbf{b} = \mathbf{0}$, which forms a polyhedral cone (shaded blue), and then add the constraint $x_1 + x_2 = 1$. The intersection of the cone and  $x_1 + x_2 = 1$ form a line segment.
...
\end{minipage}%
\begin{minipage}[t][][b]{.6\linewidth}
\begin{center} \includegraphics[width=3.07in]{figs/Section2-tikz12.png}\par\captionof{figure}{Recession cone plot: shaded blue cone between the lines s1 = 0 (red) and s3 = 0 (orange) with s2 =...}\label{fig:Section2-tikz13}% fig-num-auto
 \end{center}
\end{minipage}

% [duplicate figure removed 2026-08-24: identical to the panel above (fig:Section2-tikz13)]
...
\begin{center}  \includegraphics[width=4.80in]{figs/Section2-tikz13.png}\par\captionof{figure}{Zoomed view of the normalization segment: the line x1 + x2 = 1 crosses the shaded cone between s1 =...}\label{fig:Section2-tikz15}% fig-num-auto
 \end{center}
